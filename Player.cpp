...
void Player::Update(ViewProjection& viewProjection_) {
	...
	//	Matrix4x4 matInverseView = Inverse(viewProjection_.matView);
	//	Matrix4x4 matInverseProjection = Inverse(viewProjection_.matProjection);

	//	// 合成行列の逆行列を計算する
	//	Matrix4x4 matInverseVPV = Multiply(Multiply(matInverseViewport,matInverseProjection),matInverseView);
	...
	ScreenWorldTransformation(viewProjection_);
